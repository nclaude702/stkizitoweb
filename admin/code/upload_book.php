   ...
   //$_FILES=$_POST['myfile']['name'];
    ...
    if (!in_array($extension, ['txt','jpeg','xls','xlsx','zip','jpg','jpeg','sql','png', 'pdf', 'docx','doc','sql','sql'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
            ...
            $sql="INSERT into book VALUES('','$trade','$level','$title','$filename','$trainer','$now')";
            //UPDATE `message` SET `message` = 'hello ss' WHERE `message`.`m_id` = 1;
            if (mysqli_query($conn, $sql)) {
                
                echo "<script>window.alert('success and Thank you  to upload book ');window.location='../uploads.php'</script>'";
            }else{ echo "<script>window.alert(' Fail  try again');window.location='../uploads.php'</script>'";}
        } else {
